### Project #4 README.md

**"Computing a Mean" Questions**
	![Chart of Ration between Speedup Ration and Number of Threads](project2_chart.pdf)

	*Ratio converges to around 22.30. The maximum speedup value I got is 22.26. When I use more cores than avvailable, I lose speed a little over a quarter of the avaiable cores.

	*The scaling does start off linear but once it reaches a certain number of threads, it gradually tapers off.

    *

    *
