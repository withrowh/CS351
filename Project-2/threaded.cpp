...
int main(int argc, char* argv[]) {
    ...
    size_t chunkSize = 1 + (data.size() / numThreads);
    ...
    for (size_t id = 0; id < threads.size(); ++id) {
        threads[id] = std::jthread(
            [&, id]() {
	    //Re-evaluating start and stopping points for every thread
	    size_t startPoint = id * chunkSize;
	    size_t stopPoint = std::min(data.size(), startPoint+chunkSize);
	    	for(size_t i = startPoint; i < stopPoint; ++i){
                	sums[id] += data[i];
		}
